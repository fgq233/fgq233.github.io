...
public void save(boolean highlight) {
    ...
    if (highlight) {
        // 包含高亮查询的话，必须指定 query 类型为 match
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();
        boolQueryBuilder.should().add(QueryBuilders.matchQuery("name", "外滩"));
        boolQueryBuilder.should().add(QueryBuilders.matchQuery("address", "外滩"));
        boolQueryBuilder.should().add(QueryBuilders.matchQuery("business", "外滩"));
        functionScoreQueryBuilder = QueryBuilders.functionScoreQuery(boolQueryBuilder, builders)
