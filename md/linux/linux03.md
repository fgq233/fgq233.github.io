...
| 语法   | 作用         | 说明 |示例 |
| ------ | ----------| ---- |---- |
| `grep [-n] 搜索内容 文件路径` | 搜索包含目标内容的行 | `-n`用于在结果中显示行号 | `grep -n 'fgq' a.txt` |
| `wc [-c -m -l -w] 文件路径` | 统计 | `-c`统计文件大小<br>`-m`统计字符数量<br>`-l`统计行数<br>`-w`统计单词数量<br> | `wc -c a.txt`<br>`wc -m a.txt`<br>`wc -l a.txt`<br>`wc -w a.txt`<br> |
